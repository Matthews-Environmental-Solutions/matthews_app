...
import {
  ...
  UserInfoHandler
} from './user-info-request-handler';
import {
  IonicEndSessionHandler,
  EndSessionHandler
} from './end-session-request-handler';
import { IAuthConfig } from './auth-configuration';
import {
  IonicAuthorizationRequestHandler,
  AUTHORIZATION_RESPONSE_KEY
} from './authorization-request-handler';
import { AuthBrowser, DefaultBrowser } from './auth-browser';
import {
  StorageBackend,
  Requestor,
...
import {
  ...
  EndSessionRequest
} from './end-session-request';
import { Observable, BehaviorSubject } from 'rxjs';
import { take } from 'rxjs/operators';
import {
  ImplicitRequestHandler,
  ImplicitNotifier,
  IMPLICIT_RESPONSE_KEY
} from './implicit-request-handler';
import {
  ImplicitRequest,
  ImplicitRequestJson,
  ImplicitResponseType
} from './implicit-request';
...
export class IonicAuth implements BaseIonicAuth {
  ...
  protected authSubject: BehaviorSubject<IAuthAction> = new BehaviorSubject<
    IAuthAction
  >(AuthActionBuilder.Default());
  public authObservable: Observable<
    IAuthAction
  > = this.authSubject.asObservable();

  constructor(
    protected browser: AuthBrowser = new DefaultBrowser(),
    protected storage: StorageBackend = new LocalStorageBackend(),
    protected requestor: Requestor = new JQueryRequestor(),
    protected tokenHandler: TokenRequestHandler = new BaseTokenRequestHandler(
      requestor
    ),
    protected userInfoHandler: UserInfoHandler = new IonicUserInfoHandler(
      requestor
    ),
    protected requestHandler:
      | AuthorizationRequestHandler
      | ImplicitRequestHandler = new IonicAuthorizationRequestHandler(
      browser,
      storage
    ),
    protected endSessionHandler: EndSessionHandler = new IonicEndSessionHandler(
      browser
    )
  ) {
    this.setupNotifier();
  ...
  protected getAuthConfig(): IAuthConfig {
    if (!this.authConfig) {
      this.authSubject.next(
        AuthActionBuilder.Error(new Error('AuthConfig Not Defined'))
      );
    }
  ...
  protected setupNotifier() {
    if (this.requestHandler instanceof AuthorizationRequestHandler) {
      ...
      const notifier = new ImplicitNotifier();
      this.requestHandler.setImplicitNotifier(notifier);
      notifier.setImplicitListener((request, response, error) =>
        this.onImplicitNotification(request, response, error)
      );
    }
  }

  protected async onImplicitNotification(
    ...
    if (response != null) {
      ...
      this.authSubject.next(AuthActionBuilder.Error(error));
    } else {
      this.authSubject.next(
        AuthActionBuilder.Error(new Error('Unknown Error With Authentication'))
      );
    }
  }
  ...
  protected onAuthorizationNotification(
    ...
    const codeVerifier: string | undefined =
      request.internal !== undefined && this.getAuthConfig().usePkce
  ...
  public async signIn(loginHint?: string, prompt?: boolean) {
    await this.performAuthorizationRequest(loginHint, prompt).catch(
      response => {
  ...
  public async getUserInfo<T>(): Promise<T> {
    const token: TokenResponse | undefined = await this.getValidToken();

    if (token !== undefined) {
      return this.userInfoHandler.performUserInfoRequest<T>(
        await this.getConfiguration(),
        token
      );
    } else {
      this.authSubject.next(
        AuthActionBuilder.Error(
          new Error('Unable To Obtain User Info - No Token Available')
        )
      );
    }
  }
  ...
  public async startUpAsync() {
    ...
    this.authObservable.subscribe((action: IAuthAction) =>
      this.authObservableEvents(action)
    );
    ...
    if (!token) {
      this.authSubject.next(
        AuthActionBuilder.AutoSignInFailed('No Token Available')
      );
    } else {
  ...
  protected async performEndSessionRequest(): Promise<void> {
    const token: TokenResponse | undefined = await this.getTokenFromObserver();
    ...
    if (token !== undefined) {
      ...
      const returnedUrl:
        | string
        | undefined = await this.endSessionHandler.performEndSessionRequest(
        await this.getConfiguration(),
        request
      );
  ...
  protected async performAuthorizationRequest(
    ...
    if (this.requestHandler instanceof AuthorizationRequestHandler) {
      return this.requestHandler.performAuthorizationRequest(
        await this.getConfiguration(),
        await this.getAuthorizationRequest(loginHint, prompt)
      );
    } else {
      return this.requestHandler.performImplicitRequest(
        await this.getConfiguration(),
        await this.getImplicitRequest(loginHint, prompt)
      );
    }
  }

  protected async getAuthorizationRequest(
    loginHint?: string,
    prompt?: boolean
  ) {
    ...
    const requestJson: AuthorizationRequestJson = {
      response_type:
        authConfig.response_type || AuthorizationRequest.RESPONSE_TYPE_CODE,
    ...
    if (prompt) {
      requestJson.extras = requestJson.extras || {};
      requestJson.extras['prompt'] = 'none';
    }
    ...
    if (authConfig.usePkce) await request.setupCodeVerifier();

    return request;
  }

  protected async getImplicitRequest(loginHint?: string, prompt?: boolean) {
    const authConfig: IAuthConfig = this.getAuthConfig();
    const requestJson: ImplicitRequestJson = {
      response_type:
        authConfig.response_type || ImplicitResponseType.IdTokenToken,
      client_id: authConfig.identity_client,
      redirect_uri: authConfig.redirect_url,
      scope: authConfig.scopes,
      extras: authConfig.auth_extras
  ...
  > {
    ...
    if (this.configuration !== undefined) {
      ...
      this.authSubject.next(
        AuthActionBuilder.Error(
          new Error('Unable To Obtain Server Configuration')
        )
      );
    }
  }
  ...
  public async getValidToken() {
    ...
    if (token === undefined) {
      this.authSubject.next(
        AuthActionBuilder.Error(
          new Error('Unable To Obtain Token - No Token Available')
        )
      );
    }
    ...
    if (authConfig.auth_extras) {
      if (authConfig.auth_extras.hasOwnProperty(IS_VALID_BUFFER_KEY)) {
        isValidBuffer = parseInt(
          authConfig.auth_extras[IS_VALID_BUFFER_KEY],
          10
        );
      }
    }
  ...
  protected async getTokenFromObserver(): Promise<TokenResponse | undefined> {
    return this.authSubject
      .pipe(take(1))
      .toPromise()
      .then((action: IAuthAction) => action.tokenResponse);
  }
