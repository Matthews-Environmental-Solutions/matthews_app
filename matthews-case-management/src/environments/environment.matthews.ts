export const environment = {
    production: true,
    apiUrl: 'https://develop.comdata.rs/MatthewsApp.API',
    i4connectedApiUrl: 'https://matthewsenvironmental.i4connected.cloud/api',
    baseUrl: '/MatthewsApp.Manager',
    identityServerClientId: 'i4connected.cremator',
    allowedUrls: [
        'https://matthewsenvironmental.i4connected.cloud/api', 
        'https://localhost:5001', 
        'https://develop.comdata.rs/MatthewsApp.API', 
        'https://matthewsenvironmental.i4connected.cloud/MatthewsApp.API'
    ],
};